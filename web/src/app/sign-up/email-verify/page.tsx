    ...
    <div className="flex flex-1 items-center justify-center">
      ...
      >
        ...
        <div className="space-y-3 flex flex-col items-center w-full mb-2">
          <div className="h-8 w-40 bg-neutral-800 rounded-lg animate-pulse" />
          <div className="h-4 w-60 bg-neutral-800 rounded-md animate-pulse" />
        </div>

        {/* OTP Slots placeholder (6 slots) */}
        <div className="flex gap-2">
          {[...Array(6)].map((_, idx) => (
            <div key={idx} className="w-12 h-14 bg-neutral-800 border-2 border-neutral-800 rounded-xl animate-pulse" />
          ))}
        </div>

        {/* Resend button placeholder */}
        <div className="h-5 w-32 bg-neutral-800 rounded-md animate-pulse mt-2" />
      </div>
    </div>
  )
}
