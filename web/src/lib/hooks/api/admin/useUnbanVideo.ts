...
import { AdminVideoRecord, unbanVideoAction } from '@/lib/actions/admin.actions'
...
export const useUnbanVideoMutation = () => {
  const addToast = useToastStore(state => state.addToast)
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: async (videoId: string) => {
      return unwrapServerAction(await unbanVideoAction(videoId))
    },
    onSuccess: (_, videoId) => {
        ...
        (oldData: InfiniteData<PaginatedData<AdminVideoRecord>> | undefined) => {
          ...
          return {
            ...
            pages: oldData.pages.map(page => ({
              ...page,
              items: page.items.map(video => (video.videoId === videoId ? { ...video, isBanned: false } : video)),
            })),
          }
        },
      )
