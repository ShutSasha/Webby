...
import { Complaint, denyComplaintAction } from '@/lib/actions/admin.actions'
import { extractServerMessage, ServerActionError, unwrapServerAction } from '@/lib/utils/general.utils'
import { removePaginatedCacheItem } from '@/lib/utils/query.utils' // Вкажи свій шлях до файлу
import { useToastStore } from '@/stores/toast-store'
import { PaginatedData } from '@/types/general.types'

type DenyComplaintVariables = {
  complaintId: string
  reason: string
}

export const useDenyComplaintMutation = () => {
  const addToast = useToastStore(state => state.addToast)
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: async ({ complaintId, reason }: DenyComplaintVariables) => {
      return unwrapServerAction(await denyComplaintAction(complaintId, reason))
    },
    onMutate: async ({ complaintId }) => {
      ...
      addToast('Complaint dismissed', 'success')
