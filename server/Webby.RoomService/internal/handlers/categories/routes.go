package categories

import (
	"log/slog"
	"net/http"
	"webby/internal/handlers/categories/create"
	"webby/internal/handlers/categories/delete"
	"webby/internal/handlers/categories/list"
	"webby/internal/handlers/categories/update"
	"webby/pkg/http/middleware/auth"
)

type CategoryService interface {
	create.Creator
	list.Lister
	update.Updater
	delete.Deleter
}

func RegisterCategories(mux *http.ServeMux, jwtSecret []byte, logger *slog.Logger,
	categoryService CategoryService) {
	requireAuth := auth.AuthMiddleware(jwtSecret)

	mux.Handle("GET /api/categories", list.New(logger, categoryService))

	mux.Handle("POST /api/categories", requireAuth(create.New(logger, categoryService)))
	mux.Handle("PUT /api/categories/{name}", requireAuth(update.New(logger, categoryService)))
	mux.Handle("DELETE /api/categories/{name}", requireAuth(delete.New(logger, categoryService)))
}
